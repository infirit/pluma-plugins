...
"""
This is searched by pluma for a class extending pluma.Plugin
"""
...
from base.decorators import PlumaWindowDecorator
from util import open_error
from preferences.dialog import PreferencesDialog


class PlumaLaTeXPlugin(pluma.Plugin):
	"""
	...
	# be sure that we're running on a pluma with the new binding API
	...
	_log = logging.getLogger("PlumaLaTeXPlugin")
	...
	def __init__(self):
		...
		requirements = [
				...
				(pluma.version, self._REQUIRED_PLUMA_VERSION, "pluma"),
	...
	def activate(self, window):
		"""
		Called when the plugin is loaded with pluma or activated in
		configuration
		...
		if self._platform_okay:
			self._window_decorators[window] = PlumaWindowDecorator(window)
