...
# This file is part of the Pluma LaTeX Plugin
#
...
These classes are 'attached' to the according pluma objects. They form the
extension point.
"""
...
# TODO: maybe create ActionDelegate for PlumaWindowDecorator


class PlumaWindowDecorator(IPreferencesMonitor):
	"""
	This class
	 - manages the PlumaTabDecorators
	 - hooks the plugin actions as menu items and tool items
	 - installs side and bottom panel views
	"""

	_log = getLogger("PlumaWindowDecorator")

	# ui definition template for hooking tools in pluma's ui
	...
	def _init_actions(self):
		"""
		Merge the plugin's UI definition with the one of pluma and hook the actions
		"""
	...
	def _init_tab_decorators(self):
		...
		for view in views:
			tab = pluma.tab_get_from_document(view.get_buffer())
	...
	def activate_tab(self, file):
		"""
		Activate the PlumaTab containing the given File or open a new
		...
		uri = file.uri
		self._log.debug("PlumaWindow.create_tab_from_uri(%s)" % uri)
		if pluma.utils.uri_is_valid(uri):
			self._window.create_tab_from_uri(file.uri, pluma.encoding_get_current(), 1, False, True)
		else:
			self._log.error("pluma.utils.uri_is_valid(%s) = False" % uri)
	...
	def adjust(self, tab_decorator):
		 ...
		 * PlumaTabDecorator when the Editor instance changes
		"""
	...
	def _on_tab_added(self, window, tab):
		...
		@param window: pluma.Window object
		@param tab: pluma.Tab object
		"""
	...
	def _on_tab_removed(self, window, tab):
		...
		@param window: PlumaWindow
		@param tab: the closed PlumaTab
		"""
		...
		# As we don't call PlumaWindowDecorator.adjust() if the new
		# tab is not the active one (for example, when opening several
		# files at once, see PlumaTabDecorator._adjust_editor()),
	...
	def _on_active_tab_changed(self, window, tab):
		...
		@param window: the PlumaWindow
		@param tab: the activated PlumaTab
		"""
		...
		if tab in self._tab_decorators.keys():
			...
			# (on pluma startup 'tab-changed' comes before 'tab-added')
	...
	def _create_tab_decorator(self, tab, init=False):
		"""
		Create a new PlumaTabDecorator for a PlumaTab
		"""
		decorator = PlumaTabDecorator(self, tab, init)
...
class PlumaTabDecorator(object):
	"""
	This monitors the opened file and manages the Editor objects
	according to the current file extension
	"""

	_log = getLogger("PlumaTabDecorator")

	def __init__(self, window_decorator, tab, init=False):
		"""
		Construct a PlumaTabDecorator

		@param window_decorator: the parent PlumaWindowDecorator
		@param tab: the PlumaTab to create this for
		...
		# listen to PlumaDocument signals
	...
	def _adjust_editor(self):
		...
		if uri is None:
			# this happends when the plugin is activated in a running pluma
			...
			if file == self._file:		# FIXME: != doesn't work for File...
				...
				if not editor_class is None:
					...
					#   File "/home/michael/.gnome2/pluma/plugins/PlumaLaTeXPlugin/src/base/decorators.py", line 662, in _on_load
					#     self._adjust_editor()
					#   File "/home/michael/.gnome2/pluma/plugins/PlumaLaTeXPlugin/src/base/decorators.py", line 716, in _adjust_editor
					#     self._editor = editor_class(self, file)
					#   File "/home/michael/.gnome2/pluma/plugins/PlumaLaTeXPlugin/src/base/__init__.py", line 353, in __init__
					#     self.init(file, self._window_context)
					#   File "/home/michael/.gnome2/pluma/plugins/PlumaLaTeXPlugin/src/latex/editor.py", line 106, in init
					#     self.__parse()
					#   File "/home/michael/.gnome2/pluma/plugins/PlumaLaTeXPlugin/src/latex/editor.py", line 279, in __parse
					#     self._outline_view.set_outline(self._outline)
					#   File "/home/michael/.gnome2/pluma/plugins/PlumaLaTeXPlugin/src/latex/views.py", line 228, in set_outline
